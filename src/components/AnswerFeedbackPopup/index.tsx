...
const AnswerFeedbackPopup = ({isOpen, type, onContinue, isLoading}: AnswerFeedbackPopupProps) => {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-50 ${isCorrect ? 'bg-green-100' : 'bg-red-100'}`}>
      <div className="w-full px-6 py-6 flex items-center justify-between">
        
        {/* Lado esquerdo: Ícone + Mensagem */}
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isCorrect ? 'bg-[#58CC02]' : 'bg-red-500'}`}>
              {isCorrect ? (
        ...
        <button 
          onClick={onContinue}
          className={`px-8 py-4 rounded-2xl font-bold text-lg uppercase shadow-lg hover:shadow-xl transition-all text-white ${isCorrect ? 'bg-[#58CC02] hover:bg-[#4cb302]' : 'bg-red-500 hover:bg-red-600'}`}
        >
          Continuar
        </button>
      </div>
